...
async function run() {
  ...
  try {
    ...
    const verifiedAgent = async (req, res, next) => {
      const email = req?.user?.email;
      const user = await userDataCollection.findOne({ email });
      if (!user || user?.roll !== "agent")
        return res.status(403).send({ message: "Agent only Actions!" });
    ...
    app.post("/jwt", async (req, res) => {
      ...
      // Send token in JSON response only — frontend stores it in localStorage
...
app.get("/properties", async (req, res) => {
  try {
    const { search = "", sortBy = "MinimumPrice", sort = "asc" } = req.query;

    const allowedSortFields = ["MinimumPrice", "MaximumPrice"];
    const sortField = allowedSortFields.includes(sortBy) ? sortBy : "MinimumPrice";
    const sortOrder = sort === "desc" ? -1 : 1;

    const pipeline = [
      {
        $addFields: {
          MinimumPriceNum: { $toDouble: "$MinimumPrice" },
          MaximumPriceNum: { $toDouble: "$MaximumPrice" },
        },
      },
      {
        $match: search
      ...
      {
        $sort: {
          [`${sortField}Num`]: sortOrder,
        },
      },
    ];

    const properties = await propertiesCollection.aggregate(pipeline).toArray();

    // Remove numeric helper fields before sending
    const cleaned = properties.map(({ MinimumPriceNum, MaximumPriceNum, ...rest }) => rest);
    ...
    app.patch("/makeOffer/payment-success/:offerId", async (req, res) => {
      ...
      try {
        ...
        const result = await userMakeOfferCollection.updateOne(
          filter,
          updateDoc
        );
    ...
    app.post("/order", async (req, res) => {
      ...
      if (!orderData || !orderData.offerAmount || !orderData.transactionId) {
        return res
          .status(400)
          .send({ message: "Missing required order fields" });
      }
    ...
    app.get("/sold-properties/:agentEmail", async (req, res) => {
      ...
      try {
        ...
        const sold = await ordersCollection
          .find({
            agentEmail,
            status: "pending", // Only return sold ones
          })
          .toArray();
    ...
    app.get("/users", verifyToken, async (req, res) => {
      ...
      const user = await userDataCollection.find(filter).toArray();
      res.send(user);
    });
    ...
    app.patch("/user/roll/:email", verifyToken, async (req, res) => {
      ...
      try {
        ...
        // Step 2: If fraud, update all related properties to status "fraud"
        if (roll === "fraud") {
          const updatePropertiesResult = await propertiesCollection.updateMany(
            { "agent.email": email },
            { $set: { status: "fraud" } }
          );

          // Step 3: Delete those properties after marking as fraud
          ...
          return res.send({
            userUpdateResult,
            updatedPropertiesCount: updatePropertiesResult.modifiedCount,
            deletedPropertiesCount: deletePropertiesResult.deletedCount,
            message:
              "User marked as fraud. Related properties were updated and deleted.",
          });
        }
